        ...
        if (f0.virtualFingerIndex != -1 && f1.virtualFingerIndex != -1) {
            if (clampedFingerCount == 4) {
                ...
                for (int j = 2; j < clampedFingerCount; j++) {
                    ...
                    clip_no_update_limits(fj.x, mt_interface->logical_min_x, mt_interface->logical_max_x, margin_size_x);
                    clip_no_update_limits(fj.y, mt_interface->logical_min_y, mt_interface->logical_max_y, margin_size_y);
                }
            }
                ...
                clip_no_update_limits(f2.x, mt_interface->logical_min_x, mt_interface->logical_max_x, margin_size_x);
                clip_no_update_limits(f2.y, mt_interface->logical_min_y, mt_interface->logical_max_y, margin_size_y);
            }
        }
        else
...
void ApplePS2SynapticsTouchPad::sendTouchData() {
    ...
    for(int i = 0; i < SYNAPTICS_MAX_FINGERS; i++) {
        ...
        clip(posX, mt_interface->logical_min_x, mt_interface->logical_max_x, margin_size_x, dimensions_changed);
        clip(posY, mt_interface->logical_min_y, mt_interface->logical_max_y, margin_size_y, dimensions_changed);
